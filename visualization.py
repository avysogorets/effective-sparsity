...
parser.add_argument('--num_samples',type=int,default=3,help='number of seeds to average')
parser.add_argument('--pruners_to_display',nargs='+',default=[],help='select pruners for display')
parser.add_argument('--out_path',type=str,help='path to the folder with output files')
parser.add_argument('--pruning_type',type=str,default='effctive',help='pruning type used to generate output files')
parser.add_argument('--effective_method',type=str,default='synflow',help='method to compute effective compression (custom or synflow)')
...
pruners=['random/erk','random/igq','magnitude/gobal','snip','snip/iterative','random/snip','synflow','random/synflow','random/uniform','lamp','magnitude/pistons','magnitude/uniform','magnitude/erk','magnitude/uniform_plus','random/uniform_plus']
...
color_map={'random/uniform_plus':'blue','random/snip':'limegreen','lamp':'aqua','synflow':'orange','snip':'blue','random/uniform':'mediumorchid','snip/iterative':'cyan','random/erk':'green','random/igq':'red','magnitude/uniform_plus':'blue','magnitude/global':'gold','random/synflow':'orange','magnitude/uniform':'mediumorchid','magnitude/igq':'red','magnitude/erk':'limegreen'}
...
def accuracies(args):
    ...
    counts=np.load(os.path.join(args.out_path,args.architecture,f'dense/{args.pruning_type}_0_1_counts.npy'))
    accuracies={pruner:np.array([[np.load(os.path.join(load_prefix[pruner],f'{args.pruning_type}_{sample}_{compression}_accuracies.npy'))[-1] for compression in compressions[pruner]] for sample in range(args.num_samples)]) for pruner in args.pruners_to_display}
    direct_sparsities={pruner:np.array([[np.load(os.path.join(load_prefix[pruner],f'{args.pruning_type}_{sample}_{compression}_sparsities.npy')) for compression in compressions[pruner]] for sample in range(args.num_samples)]) for pruner in args.pruners_to_display}
    effective_sparsities={pruner:np.array([[np.load(os.path.join(load_prefix[pruner],f'{args.pruning_type}_{sample}_{compression}_sparsities_effective_{args.effective_method}.npy')) for compression in compressions[pruner]] for sample in range(args.num_samples)]) for pruner in args.pruners_to_display}
    ...
    dense=np.array([np.load(os.path.join(args.out_path,args.architecture,f'dense/{args.pruning_type}_{sample}_1_accuracies.npy'))[-1] for sample in range(args.num_samples)])
    ...
    for pruner in args.pruners_to_display:
        ...
        ax.fill_between(ticks[pruner][start_idx:end_idx],mins[start_idx:end_idx],maxs[start_idx:end_idx],color=color_map[pruner],zorder=3,alpha=0.2)
    ...
    ax.fill_between(plt.xlim(),dense_min,dense_max,color='grey',zorder=3,alpha=0.2)
...
def compressions(args):
    ...
    for pruner in args.pruners_to_display:
        ax.plot(compressions[pruner][:inf_idxs_effective[pruner]],overall_effective_compression_mean[pruner][:inf_idxs_effective[pruner]],color=color_map[pruner])
        ax.fill_between(compressions[pruner][:inf_idxs_effective[pruner]],overall_effective_compression_min[pruner][:inf_idxs_effective[pruner]],overall_effective_compression_max[pruner][:inf_idxs_effective[pruner]])
        legend_elements+=[mpl.patches.Patch(facecolor=color_map[pruner],label=pruner_names[pruner])]
    ax.set_title(network_names[args.architecture],fontsize='medium')
