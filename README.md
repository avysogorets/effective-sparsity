...
#### Environment
To run code from this project, first clone the repository and install dependencies by executing ```pip install -r requirements.txt```. The code was tested for Python-3.6 and Python-3.8 with TensorFlow-2.0.0. Note that ```tensorflow-gpu``` currently requires exactly ```tensorflow==2.0.0``` as a dependency.
